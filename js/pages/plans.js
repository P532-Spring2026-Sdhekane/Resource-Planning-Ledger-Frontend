...
async function refreshPlan() {
  ...
  document.getElementById("plan-detail-status").innerHTML = badge(
...
async function selectAction(id) {
  ...
  document
    .getElementById("action-detail-card")
...
async function refreshAction() {
  ...
  btnsEl.innerHTML = (TRANSITIONS[status] ?? [])
    .map(
      ([ev, label, cls]) =>
  ...
  const allocTbody = document.getElementById("action-alloc-tbody");
  if (allocs.length === 0) {
    allocTbody.innerHTML = `<tr><td colspan="5" class="empty">No allocations yet.</td></tr>`;
  } else {
    allocTbody.innerHTML = allocs
      .map(
        (a) => `<tr>
      ...
      .join("");
    allocTbody.querySelectorAll("[data-alloc-id]").forEach((btn) => {
      btn.onclick = async () => {
        try {
          await api.deleteAllocation(
            selectedActionId,
            parseInt(btn.dataset.allocId),
          );
          await refreshAction();
        } catch (err) {
          alert("Error: " + err.message);
  ...
  try {
    ...
    sel.innerHTML =
      rts.length === 0
        ...
        : rts
            .map(
              (r) => `<option value="${r.id}">${r.name} (${r.kind})</option>`,
            )
            .join("");
  } catch (_) {}
}
...
async function populateProtocolOptions() {
  try {
    const protos = (await api.getProtocols)
      ? api.getProtocols()
      : Promise.resolve([]);
